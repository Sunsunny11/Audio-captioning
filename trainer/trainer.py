...
from tools.main_augment import Text_Augmentation
...
def train(config):
    ...
    spiders = []
    src_vlad = []
    # Text Augmentation initialization
    if config.Text_Augment.Augmentation:
        text_augmentation = Text_Augmentation(config)    # need to initialization

    # batch_generate = Text_Augmentation.batch_generate
    for epoch in range(ep, config.training.epochs + 1):
        ...
        for batch_id, batch_data in tqdm(enumerate(train_loader), total=len(train_loader)):
            ...
            if config.Text_Augment.Augmentation:
                captions = text_augmentation.batch_generate(captions)
            ...
            y_hat = model(src, tgt, src_vlad, target_padding_mask=tgt_pad_mask)
            tgt = tgt[:, 1:]  # exclude <sos>
            y_hat1 = y_hat['output1']
            y_hat2 = y_hat['output2']

            y_hat1 = y_hat1.transpose(0, 1)  # batch x words_len x ntokens
            y_hat1 = y_hat1[:, :tgt.size()[1], :]  # truncate to the same length with target

            y_hat2 = y_hat2.transpose(0, 1)  # batch x words_len x ntokens
            y_hat2 = y_hat2[:, :tgt.size()[1], :]

            y_hat1 = y_hat1.contiguous().view(-1, y_hat1.size()[-1])

            y_hat2 = y_hat2.contiguous().view(-1, y_hat2.size()[-1])
            ...
            loss = criterion(y_hat, tgt.contiguous().view(-1))
