...
def beam_decode(src, model, sos_ind, eos_ind, beam_width=5, top_k=1, max_len=30):
    ...
    for idx in range(batch_size):
        ...
        encoded_feat = encoded_feats['src6'][:, idx, :].unsqueeze(1)
        encoded_feat2 = encoded_feats['src'][:, idx, :].unsqueeze(1)
        ...
        while time_step < max_len:
            ...
            decoder_output = model.decode(encoded_feat, decoder_input, encoded_feat2)
            #log_prob = F.log_softmax(decoder_output[-1, :], dim=-1)
            log_prob1 = F.log_softmax(decoder_output['output1'][-1, :], dim=-1)
            log_prob2 = F.log_softmax(decoder_output['output2'][-1, :], dim=-1)
